...
Hdf5dir = r'HDF5data'
train_or_test = 'train'
F = h5py.File(os.path.join(Root_dir, Hdf5dir, 'train_youku_multi.hdf5'), 'w')
...
def generate(y, u, v, im_size, patch_size, step, name):
    [hgt, wdt] = im_size
    ...
    for path in sr_dir:
        ...
        for name in sr_name_list:
            ...
            for i in range(0, Total_frame-3, 2):
                ...
                # print('='*20)
                # print(v_sr.shape)
                generate(y_sr, u_sr, v_sr, [Height,Width], patch_size=Sr_size, step=Sr_step, name='sr_{}_{}'.format(Sr_size, Sr_size))
                generate(y_lr, u_lr, v_lr, [Height>>2,Width>>2], patch_size=Lr_size, step=Lr_step, name='lr_{}_{}'.format(Lr_size, Lr_size))
